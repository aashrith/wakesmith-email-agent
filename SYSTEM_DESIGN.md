...
2. **Email transport**: a self-hosted Docker mail container (GreenMail or smtp4dev — both give real SMTP + IMAP, unlike Mailpit which lacks IMAP) hosting two mailboxes, "MarseerAI outreach" and "prospect." Real protocol send/receive, fully reproducible via `docker compose up` — no external credentials needed to re-run the demo.
