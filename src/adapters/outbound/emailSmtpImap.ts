...
export class SmtpImapEmailGateway implements EmailGateway {
    ...
    await withRetry(
        ...
        }),
      {
        ...
        isRetryable: (err) => {
          ...
          if (retryable) logger.warn("smtp send failed, retrying", { to: toAddress, error: String(err) });
          return retryable;
        },
      },
    );
  ...
  async fetchNew(): Promise<InboundEmail[]> {
    ...
    await withRetry(() => client.connect(), {
      ...
      isRetryable: (err) => {
        const retryable = isTransientNetworkError(err);
        if (retryable) logger.warn("imap connect failed, retrying", { error: String(err) });
        return retryable;
      },
    });
    try {
